                ...
                <div class="card">
                    ...
                    <div class="card-body">
                           ...
                           <c:forEach items="${venta.items}" var="item">
                               <tr>
                                   <td>
                                       ...
                                       <!-- Mostrar si fue devuelto parcial/total -->
                                       <c:if test="${item.cantidadDevuelta > 0}">
                                           <br>
                                           <small class="text-warning">
                                               <i class="fas fa-undo"></i>
                                               Devuelto: ${item.cantidadDevuelta} de ${item.cantidad}
                                           </small>
                                       </c:if>
                                   </td>
                                   <td class="text-center">
                                       ...
                                       <c:if test="${item.cantidadDevuelta > 0}">
                                           <br>
                                           <small class="text-muted">(-${item.cantidadDevuelta})</small>
                                       </c:if>
                                   </td>
                                   <td class="text-end">
                        ...
                        <c:if test="${venta.estado == 'COMPLETADA'}">
                            <div class="mt-3">
                                <a href="${pageContext.request.contextPath}/devoluciones/nueva/${venta.id}"
                        ...
                        <!-- BOTÓN ANULAR (solo si no hay devoluciones aprobadas) -->
                        <c:if test="${venta.estado == 'COMPLETADA'}">

                            <!-- Verificar si hay devoluciones -->
                            <c:set var="tieneDevolucionesAprobadas" value="false"/>
                            <c:forEach items="${venta.items}" var="item">
                                <c:if test="${item.cantidadDevuelta > 0}">
                                    <c:set var="tieneDevolucionesAprobadas" value="true"/>
                                </c:if>
                            </c:forEach>

                            <c:choose>
                                <c:when test="${tieneDevolucionesAprobadas}">
                                    <div class="alert alert-info mt-3">
                                        <i class="fas fa-info-circle"></i>
                                        Esta venta tiene devoluciones aprobadas.
                                        <a href="${pageContext.request.contextPath}/devoluciones?ventaId=${venta.id}">
                                            Ver devoluciones
                                        </a>
                                    </div>
                                </c:when>
